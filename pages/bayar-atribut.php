  ...
  <div class="col-md-10 offset-md-1">
    <div class="card card-primary">
      <div class="card-header">
        <h3 class="card-title">Pembayaran Atribut</h3>
...
<?php
$proses = mysqli_query($koneksi,'select * from bayar_atribut order by sisa_bayar')or die(mysqli_error($koneksi));
$j=0;
echo "<script>var cek = [";
...
if(isset($_POST['submit'])){
    ...
    $query = mysqli_query($koneksi,'insert into bayar_atribut values("'.$id.'","'.$nama.'","'.$total.'","'.$terbayar.'","'.$sisa.'")') or die(mysqli_error($koneksi));
    $query1 = true;
    if ($param == 0) {
        $query1 = mysqli_query($koneksi, 'insert into tbl_bayar values("'.$id.'","'.$nama.'","'.$tanggal.'","ATRIBUT","'.$komentar.'")') or die(mysqli_error($koneksi));
    }
    if($query && $query1){
        ?> <script>
        location.replace('index.php?url=bayar-atribut');
...
function siswa(test){
  ...
  for(var i=0;i<byr.length;i++){
    if(byr[i][0] == test && byr[i][1]=='ATRIBUT'){
  ...
  if(cek.length == 0) {
    for(var i=0;i<data.length;i++){
      ...
      if(data[i][2] == param && data[i][0]=="ATRIBUT"){
    ...
    for(var i=0;i<cek.length;i++){
      if(cek[i][0] == test){
        ...
        for(var j=0;j<data.length;j++){
          console.log(data[i][2]);
          if(data[j][2] == param && data[j][0]=="ATRIBUT"){
