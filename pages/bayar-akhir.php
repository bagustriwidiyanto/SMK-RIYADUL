...
<?php
$proses = mysqli_query($koneksi,'select * from bayar_akhir order by sisa_bayar')or die(mysqli_error($koneksi));
$j=0;
echo "<script>var cek = [";
...
if(isset($_POST['submit'])){
    ...
    $query1 = true;
    if ($param == 0) {
        $query1 = mysqli_query($koneksi, 'insert into tbl_bayar values("'.$id.'","'.$nama.'","'.$tanggal.'","AKHIR","'.$komentar.'")') or die(mysqli_error($koneksi));
    }
    if($query && $query1){
        ?> <script>
